...
export class ElsaClientService {

  public eventBus = eventBus;
 public createHttpClient = async function (baseAddress: string): Promise<AxiosInstance> {
    if (!!_httpClient)
      return _httpClient;
    ...
    await this.eventBus.emit(EventTypes.HttpClientConfigCreated, this, { config });
    await this.eventBus.emit(EventTypes.HttpClientCreated, this, { service, _httpClient });

    return _httpClient;
  }

  public createElsaClient = async function (serverUrl: string): Promise<ElsaClient> {

    if (!!_elsaClient)
      return _elsaClient;

    const httpClient: AxiosInstance = await this.createHttpClient(serverUrl);
