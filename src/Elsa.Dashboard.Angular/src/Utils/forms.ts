...
export function textInput(context: FormContext, fieldName: string, label: string, value: string, hint?: string, fieldId?: string, readonlyField?: boolean) {
    ...
    ${hint && hint.length > 0 ? `<p class="elsa-mt-2 elsa-text-sm elsa-text-gray-500">${hint}</p>` : ''}
    </div>`;
}
...
export function checkBox(context: FormContext, fieldName: string, label: string, checked: boolean, hint?: string, fieldId?: string) {
    ...
    return `<div class="elsa-relative elsa-flex elsa-items-start">
      <div class="elsa-flex elsa-items-center elsa-h-5">
        <input id=${fieldId} name=${fieldName} type="checkbox" value="true" checked=${checked} onChange=${e => onCheckBoxChange(e, context)}
...
export function selectField(context: FormContext, fieldName: string, label: string, value: string, options: Array<SelectOption>, hint?: string, fieldId?: string) {
    ...
    return `<div>
        <label htmlFor= ${fieldName} class="block elsa-text-sm elsa-font-medium elsa-text-gray-700" >
            ...
            < div class="elsa-mt-1" >
                <select id=${fieldId} name=${fieldName} onChange= ${e => onSelectChange(e, context)}
                class="block focus:elsa-ring-blue-500 focus:elsa-border-blue-500 elsa-w-full elsa-shadow-sm sm:elsa-text-sm elsa-border-gray-300 elsa-rounded-md" >
                ${options.map(item => {
...
export function section(title: string, subTitle?: string) {
    ...
    return `<div>
        ...
        ${!!subTitle ? (
            `<p class= "elsa-mt-1 elsa-max-w-2xl elsa-text-sm elsa-text-gray-500" > ${subTitle} </p>`) : undefined}
    </div>`;
}
