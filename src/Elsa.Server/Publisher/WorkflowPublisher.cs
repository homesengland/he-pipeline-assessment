...
using Elsa.Server.Stores.Specifications;
using Elsa.Services;
using MediatR;
using NodaTime;
...
namespace Elsa.Server.Publisher
{
    public class WorkflowPublisher : IWorkflowPublisher
    {
        ...
        public async Task<WorkflowDefinition> RetractAsync(WorkflowDefinition workflowDefinition, CancellationToken cancellationToken = default)
        {
            ...
            await _workflowDefinitionStore.Unpublish(workflowDefinition, cancellationToken);
        ...
        public async Task<WorkflowDefinition> SaveDraftAsync(WorkflowDefinition workflowDefinition, CancellationToken cancellationToken = default)
        {
            ...
            await _workflowDefinitionStore.RemoveLatest(draft, cancellationToken);
        ...
        public async Task DeleteAsync(string workflowDefinitionId, VersionOptions versionOptions, CancellationToken cancellationToken = default)
        {
            ...
            {
                await _workflowInstanceStore.DeleteManyAsync(new Persistence.Specifications.WorkflowInstances.WorkflowDefinitionIdSpecification(workflowDefinitionId), cancellationToken);
                ...
                if (definition != null)
                {
                    if (definition.IsLatest)
                    {
                        var otherVersions = await _workflowDefinitionStore.FindManyAsync(
                                                                              new Persistence.Specifications.WorkflowDefinitions.WorkflowDefinitionIdSpecification(workflowDefinitionId, VersionOptions.All),
