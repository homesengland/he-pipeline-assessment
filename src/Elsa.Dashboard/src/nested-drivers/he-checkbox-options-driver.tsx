...
export class HeCheckboxOptionsDriver implements HePropertyDisplayDriver {
  ...
  display(activity: ActivityModel, property: ActivityPropertyDescriptor) {
    ...
    return <he-checkbox-options-property key={`${keyId}`} activityModel={activity} propertyDescriptor={property} propertyModel={prop} />;
  }

  displayNested(activity: ActivityModel, property: NestedProperty, onExpressionChanged: Function) {
    const keyId = getUniversalUniqueId();
    return <he-checkbox-options-property key={`${keyId}`} activityModel={activity} propertyDescriptor={property.descriptor} propertyModel={property.value} onExpressionChanged={e => onExpressionChanged(e, property)} />;
  }
}
