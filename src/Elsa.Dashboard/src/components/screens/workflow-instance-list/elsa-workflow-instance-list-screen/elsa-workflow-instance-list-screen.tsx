...
export class ElsaWorkflowInstanceListScreen {
  ...
  async componentWillLoad() {
    ...
    let bulkActions: Array<DropdownButtonItem> = [{
  ...
  renderBulkActions() {
    ...
    return <elsa-dropdown-button text={t('BulkActions.Title').toString()} items={actions} icon={bulkActionIcon}
  ...
  renderWorkflowFilter() {
    ...
    const allItem: DropdownButtonItem = { text: t('Filters.Workflow.All').toString(), value: null, isSelected: !selectedWorkflowId};
  ...
  renderStatusFilter() {
    ...
    const items: Array<DropdownButtonItem> = statuses.map(x => {
      const text = x ?? t('Filters.Status.All').toString();
