...
export class HeValidationProperty implements ISharedComponent, IDisplayToggle {
  ...
  render() {
    ...
    const renderCaseEditor = (validationRule: NestedActivityDefinitionProperty, index: number) => {  
      ...
      const textSyntax = validationRule.syntax;
      const conditionSyntax = SyntaxNames.JavaScript;
      const textExpression = validationRule.expressions[textSyntax];
      const conditionExpression = validationRule.expressions[TextActivityOptionsSyntax.Condition];
      const urlExpression = validationRule.expressions[TextActivityOptionsSyntax.Url] ?? "https://www";
      const useValidation: boolean = validationRule.expressions[ValidationSyntax.UseValidation] == 'true';
      const guidanceChecked = validationRule.expressions[TextActivityOptionsSyntax.Guidance] == 'true';
      const hyperlinkChecked = validationRule.expressions[TextActivityOptionsSyntax.Hyperlink] == 'true';
      ...
      return (
        <tbody key={this.keyId}>
          ...
          <tr>
            <th
              class="elsa-px-6 elsa-py-3 elsa-text-left elsa-text-xs elsa-font-medium elsa-text-gray-500 elsa-tracking-wider elsa-w-2/12">Is <br />Use Validation
            </th>
            <td class="elsa-py-0">
              <input name="choice_input" type="checkbox" checked={useValidation} value={validationRule.expressions[ValidationSyntax.UseValidation]}
                onChange={e => this._base.UpdateCheckbox(e, validationRule, ValidationSyntax.UseValidation)}
