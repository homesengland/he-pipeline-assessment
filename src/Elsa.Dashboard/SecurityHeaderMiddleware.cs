...
namespace Elsa.Dashboard
{
  public class SecurityHeaderMiddleware
  {
    ...
    public async Task Invoke([NotNull] HttpContext context, [NotNull] NonceConfig nonceConfig)
    {
      ...
      // Use wildcards to support any version from these CDNs
      var unpkgCdn = "https://unpkg.com";
      var jsDelivrCdn = "https://cdn.jsdelivr.net";

      var connectSrc = $"connect-src 'self' {elsaServer} {oAuthToken} {oAuthToken2} {unpkgCdn} {jsDelivrCdn};";
      var defaultSrc = $"default-src 'self';";
      var scriptSrc = $"script-src 'self' 'strict-dynamic' '{govUkSetupNonce}' '{elsaSetupNonce}' 'unsafe-eval' {unpkgCdn} {jsDelivrCdn};";
