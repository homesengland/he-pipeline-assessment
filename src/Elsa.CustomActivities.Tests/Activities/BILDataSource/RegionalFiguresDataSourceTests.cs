...
{
    public class RegionalFiguresDataSourceTests
    {
        [Theory]
        [AutoMoqData]
        public async Task OnExecute_ReturnsSuspendResult(BILRegionalFiguresDataSource sut)
        {
            //Arrange
        ...
        public async Task ResumeAsync_ReturnsOutcomeResult(
            ...
            string dataString,
            BilRegionalFigures data,
            BILRegionalFiguresDataSource sut)
        {
            //Arrange
            sut.Output = null;
            var context = new ActivityExecutionContext(default!, default!, default!, sut.Id, default, default);
            client.Setup(x => x.GetRegionalFigures(It.IsAny<string>())).ReturnsAsync(dataString);
            jsonHelperMock.Setup(x => x.JsonToRegionalFigures(dataString)).Returns(data);
            ...
            BILRegionalFiguresDataSource sut)
        {
            ...
            sut.Output = null;
            var context = new ActivityExecutionContext(default!, default!, default!, sut.Id, default, default);
            client.Setup(x => x.GetRegionalFigures(It.IsAny<string>())).ReturnsAsync((string?)null);
