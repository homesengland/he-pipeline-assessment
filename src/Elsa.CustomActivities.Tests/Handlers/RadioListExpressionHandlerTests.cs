...
{
    public class RadioListExpressionHandlerTests
    {

        [Theory, AutoMoqData]
        public void RadioListExpressionHandlerInheritsFromCorrectBaseClass(RadioExpressionHandler sut)
        {
            //Arrange
        ...
        [Theory, AutoMoqData]
        public void ExpressionHandlerUsesCorrectSyntax_GivenDefaultValuesUsed(RadioExpressionHandler sut)
        {
            //Arrange
        ...
        [Theory, AutoMoqData]
        public async void EvaluateAsync_ReturnsRadioListData_WithCorrectNumberOfRecords(
                        Mock<IServiceProvider> provider,
                        Mock<IExpressionEvaluator> evaluator,
                        Mock<IContentSerializer> serialiser,
            Mock<ILogger<IExpressionHandler>> logger)
        {
            ...
            radioListProperties.Add(sampleElsaProperty1);
            radioListProperties.Add(sampleElsaProperty2);
            radioListProperties.Add(sampleElsaProperty3);

            string expressionString = JsonConvert.SerializeObject(radioListProperties);

            var context = new ActivityExecutionContext(provider.Object, default!, default!, default!, default, default);

            serialiser.Setup(x => x.Deserialize<List<ElsaProperty>>(expressionString)).Returns(radioListProperties);
            ...
            RadioExpressionHandler sut = new RadioExpressionHandler(logger.Object, serialiser.Object);

            //Act

            var results = await sut.EvaluateAsync(expressionString, typeof(TextModel), context, CancellationToken.None);
            RadioModel? expectedResults = results.ConvertTo<RadioModel>();

            //Assert
            Assert.True(!expectedResults!.Choices.IsNullOrEmpty());
            Assert.Equal(radioListProperties.Count(), expectedResults!.Choices.Count());
...
Mock<IContentSerializer> serializer,
            ...
            Mock<ILogger<IExpressionHandler>> logger)
        {
            ...
            RadioExpressionHandler handler = new RadioExpressionHandler(logger.Object, serializer.Object);
