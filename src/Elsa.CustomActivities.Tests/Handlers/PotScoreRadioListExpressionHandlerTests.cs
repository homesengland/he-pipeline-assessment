...
{
    public class PotScoreRadioListExpressionHandlerTests
    {

        [Theory, AutoMoqData]
        public void PotScoreRadioListExpressionHandlerInheritsFromCorrectBaseClass(PotScoreRadioExpressionHandler sut)
        {
            //Arrange
        ...
        public void ExpressionHandlerUsesCorrectSyntax_GivenDefaultValuesUsed(PotScoreRadioExpressionHandler sut)
        {
            ...
            //Assert
            Assert.Equal(CustomSyntaxNames.PotScore, sut.Syntax);
        }

        [Theory, AutoMoqData]
        public async void EvaluateAsync_ReturnsRadioModelData(
        ...
        [Theory, AutoMoqData]
        public async void EvaluateAsync_ReturnsPotScoreRadioListData_WithCorrectNumberOfRecords(
                        Mock<IServiceProvider> provider,
                        Mock<IExpressionEvaluator> evaluator,
                        Mock<IContentSerializer> serialiser,
            Mock<ILogger<IExpressionHandler>> logger)
        {
            ...
            PotScoreRadioExpressionHandler sut = new PotScoreRadioExpressionHandler(logger.Object, serialiser.Object);

            //Act

            var results = await sut.EvaluateAsync(expressionString, typeof(TextModel), context, CancellationToken.None);
            PotScoreRadioModel? expectedResults = results.ConvertTo<PotScoreRadioModel>();
...
Mock<IContentSerializer> serializer,
        ...
        Mock<ILogger<IExpressionHandler>> logger)
        {
            ...
            string potscoreValue = "High";

            PotScoreRadioExpressionHandler handler = new PotScoreRadioExpressionHandler(logger.Object, serializer.Object);

            ElsaProperty property = SampleElsaProperty(GetDictionary(SyntaxNames.Literal, "Sample Text", prePopulatedValue: prePopulatedValue.ToString(), potScoreValue: potscoreValue), SyntaxNames.Literal, "Checkbox Text");

            //Act
            string actualPotScore = handler.EvaluatePotScore(property);

            //Assert
            Assert.Equal(potscoreValue, actualPotScore);
            ...
            Mock<ILogger<IExpressionHandler>> logger)
        {
            ...
            //Act
            string actualPotScoreNoKey = handler.EvaluatePotScore(propertyWithNoKey);
            string actualPotScoreEmptyValue = handler.EvaluatePotScore(propertyWithInvalidValue);

            //Assert
            Assert.Equal(string.Empty, actualPotScoreNoKey);
            Assert.Equal(string.Empty, actualPotScoreEmptyValue);
