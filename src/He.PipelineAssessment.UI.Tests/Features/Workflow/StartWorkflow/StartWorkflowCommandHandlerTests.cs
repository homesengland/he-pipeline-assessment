...
namespace He.PipelineAssessment.UI.Tests.Features.Workflow.StartWorkflow
{
    public class StartWorkflowCommandHandlerTests
    {
        ...
        {
            //Arrange
            roleValidation.Setup(x => x.ValidateRole(command.AssessmentId, command.WorkflowDefinitionId))
                .ReturnsAsync(true);
            ...
            assessmentRepository.Setup(x =>
                    x.GetAssessmentToolInstanceNextWorkflowByAssessmentId(command.AssessmentId,
            ...
            //Assert
            assessmentRepository.Verify(x => x.CreateAssessmentToolWorkflowInstance(
                It.Is<AssessmentToolWorkflowInstance>(y =>
        ...
        {
            //Arrange
            roleValidation.Setup(x => x.ValidateRole(command.AssessmentId, command.WorkflowDefinitionId)).ReturnsAsync(false);
