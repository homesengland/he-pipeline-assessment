...
namespace He.PipelineAssessment.UI.Tests.Features.Workflow.SaveAndContinue
{
    public class QuestionScreenSaveAndContinueMapperTests
    {
        ...
        )
        {
            ...
            Assert.Equal(saveAndContinueCommand.Data.Questions?.SelectMany(x => x.Answers.Select(y => new Answer(x.QuestionId, y.AnswerText, x.Comments, x.DocumentEvidenceLink,y.ChoiceId))).ToList(), result.Answers);
        }
    }
}
